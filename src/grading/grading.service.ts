...
export class GradingService {
  ...
  async updateCommodity(tenantId: string, id: string, dto: Partial<{
    name: string; code: string; unitOfMeasure: MeasurementUnit;
    gradingLogic: GradingLogic; numberOfGrades: number;
    standardBagWeightKg: number; description: string;
  }>) {
    const commodity = await this.prisma.commodity.findFirst({ where: { id, tenantId } });
  ...
  async getParameters(tenantId: string, commodityId: string) {
    const commodity = await this.prisma.commodity.findFirst({ where: { id: commodityId, tenantId } });
    if (!commodity) throw new NotFoundException('Commodity not found');
  ...
  async addParameter(
    tenantId: string,
    commodityId: string,
    dto: { name: string; unit: string; isDefective: boolean; thresholds: Record<string, number> },
  ) {
    const commodity = await this.prisma.commodity.findFirst({ where: { id: commodityId, tenantId } });
    if (!commodity) throw new NotFoundException('Commodity not found');

    // Validate threshold keys match numberOfGrades
    const expectedGrades = Array.from({ length: commodity.numberOfGrades }, (_, i) => `Grade ${i + 1}`);
    ...
    if (missing.length > 0) throw new BadRequestException(`Missing threshold keys: ${missing.join(', ')}`);
    if (extra.length > 0) throw new BadRequestException(`Extra threshold keys: ${extra.join(', ')}`);

    return this.prisma.gradingParameter.create({
      data: { ...dto, commodityId, tenantId, thresholds: dto.thresholds as any },
    });
  }

  async updateParameter(tenantId: string, parameterId: string, dto: Partial<{
    name: string; unit: string; isDefective: boolean; thresholds: Record<string, number>;
  }>) {
    const param = await this.prisma.gradingParameter.findFirst({ where: { id: parameterId, tenantId } });
  ...
  async applyToWarehouses(tenantId: string, commodityId: string, warehouseIds?: string[]) {
    const commodity = await this.prisma.commodity.findFirst({ where: { id: commodityId, tenantId } });
    if (!commodity) throw new NotFoundException('Commodity not found');

    const targetIds = warehouseIds?.length
      ? warehouseIds
      : (await this.prisma.warehouse.findMany({ where: { tenantId }, select: { id: true } })).map((w) => w.id);
  ...
  async scorePreview(tenantId: string, commodityId: string, measurements: Record<string, number>) {
    ...
    if (!commodity.gradingParameters.length) {
      throw new BadRequestException('This commodity has no grading parameters configured');
    }
    ...
    try {
      const result = scoreSample({ parameters: params, measurements, numberOfGrades: commodity.numberOfGrades });
