...
export class StorageFeesService {
  ...
  async getPolicies(tenantId: string, query: { warehouseId?: string; commodityId?: string; isActive?: string }) {
  ...
  async updatePolicy(tenantId: string, id: string, dto: Partial<{
    feeType: FeeType; warehouseId: string; commodityId: string;
    rate: number; billingFrequency: BillingFrequency;
    gracePeriodDays: number; latePenaltyPct: number; currency: string;
  }>) {
    const policy = await this.prisma.storageFeePolicy.findFirst({ where: { id, tenantId } });
  ...
  async activate(tenantId: string, id: string) {
    ...
    return this.prisma.storageFeePolicy.update({ where: { id }, data: { isActive: true } });
  }

  async deactivate(tenantId: string, id: string) {
    const policy = await this.prisma.storageFeePolicy.findFirst({ where: { id, tenantId } });
    if (!policy) throw new NotFoundException('Policy not found');
    return this.prisma.storageFeePolicy.update({ where: { id }, data: { isActive: false } });
  }

  // ─── FEE RESOLVER (used by withdrawals) ───────────────────────────────────

  async resolvePolicy(tenantId: string, warehouseId: string, commodityId: string) {
    const candidates = [
  ...
  calculateFee(
    ...
    const toBags = (qty: number, u: MeasurementUnit): number => {
      ...
      if (u === 'METRIC_TON') return Math.ceil((qty * 1000) / (bagWeightKg ?? 1));
      throw new BadRequestException(`Cannot convert ${u} to BAG`);
    };

    switch (policy.feeType) {
      case 'PER_MT_PER_MONTH': return toMt(quantity, unit) * policy.rate * months;
      case 'PER_MT_PER_DAY': return toMt(quantity, unit) * policy.rate * days;
      case 'PER_BAG_PER_WEEK': return toBags(quantity, unit) * policy.rate * weeks;
      case 'FLAT_RATE': return policy.rate;
    }
  }
}
